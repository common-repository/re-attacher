...
if ( ! function_exists( 'rttchr_admin_menu' ) ) {
	function rttchr_admin_menu() {
		bws_general_menu();
		add_submenu_page( 'bws_panel', 'Re-attacher ' . __( 'Settings', 're-attacher' ), 'Re-attacher', 'manage_options', "re-attacher.php", 'rttchr_settings_page' );
	}
}
/**
...
if ( ! function_exists( 'rttchr_init' ) ) {
	function rttchr_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $rttchr_plugin_info, '3.9' );
	}
}
/**
		...
		<div class="wrap">
			...
			<form method="post" action="admin.php?page=re-attacher.php" class="bws_form">
				<table class="form-table">
					<tr>
						<th><?php _e( 'Show media items only to their author', 're-attacher' ); ?></th>
						<td>
...
if ( ! function_exists( 'rttchr_show_notices' ) ) {
	function rttchr_show_notices() {
		global $hook_suffix, $rttchr_gallery_old_version, $post, $rttchr_plugin_info, $rttchr_options;
		...
		if ( in_array( $hook_suffix, $admin_pages ) || ( isset( $_GET['page'] ) && in_array( $_GET['page'], $plugin_pages ) ) ) {
			...
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				<div class="bws_banner_on_plugin_page bws_banner_to_settings">
					<div class="icon">
						<img title="" src="<?php echo plugins_url( 'images/re-attacher-icon.png', __FILE__ ); ?>" alt="" />
					</div>
					<div class="text">
						<strong><?php printf( __( '%s will be no longer supported', 'bestwebsoft' ), 'Re-attacher' ); ?></strong>
						<br />
						<?php printf( __( '%s plugin will be no longer supported (updates will be unavailable) starting from July 2017.', 'bestwebsoft' ), 'Re-attacher' ); ?>
					</div>
...
if ( ! function_exists( 'rttchr_register_plugin_links' ) ) {
	function rttchr_register_plugin_links( $links, $file ) {
		...
		if ( $file == $base ) {
			...
			$links[] = '<a href="https://support.bestwebsoft.com" target="_blank">' . __( 'FAQ', 're-attacher' ) . '</a>';
			$links[] = '<a href="https://support.bestwebsoft.com">' . __( 'Support', 're-attacher' ) . '</a>';
		}
		return $links;
	}
}
